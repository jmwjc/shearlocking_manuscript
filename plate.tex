...
\boldsymbol m \cdot \boldsymbol n &= \bar{\boldsymbol m}_n & &\textrm{on}\; \Gamma_m \\
...
\begin{equation}
    \begin{aligned}
        ...
        &+ \int_{\Gamma_w} \boldsymbol q \cdot \boldsymbol n \bar w d\Gamma + \int_{\Gamma_m} \boldsymbol \varphi \cdot \bar{\boldsymbol m}_n d\Gamma + \int_{\Gamma_q} w (\boldsymbol q \cdot \boldsymbol n - \bar q_n) d\Gamma \\&- \int_\Omega w (\nabla \cdot \boldsymbol q + \bar p) d\Omega
    \end{aligned}
\end{equation}
...
\begin{aligned}
a_\varphi(\delta \boldsymbol \varphi, \boldsymbol \varphi) + b_{\varphi}(\delta\boldsymbol \varphi,\boldsymbol q) &= f_{\varphi}(\delta \boldsymbol \varphi), & \forall \delta \boldsymbol \varphi \in \Phi \\
...
a_\varphi(\delta\boldsymbol \varphi,\boldsymbol \varphi) &= \int_\Omega \delta \boldsymbol \kappa : \frac{t^3}{12} \boldsymbol D : \boldsymbol \kappa d\Omega \\
a_q(\delta\boldsymbol q,\boldsymbol q) &= -\int_\Omega \frac{1}{2kGt} \delta \boldsymbol q \cdot \boldsymbol q d\Omega \\
% &b: (W\times \Phi)\times Q \rightarrow \mathbb R &
b_q(\boldsymbol q, \boldsymbol u) &= b_w(w, \boldsymbol q) + b_\varphi(\boldsymbol \varphi, \boldsymbol q) \\
...
f_{\varphi}(\boldsymbol \varphi) &= - \int_{\Gamma_{m}} \boldsymbol \varphi \cdot \bar{\boldsymbol m}_n d\Gamma \\
...
\begin{subequations}\label{eq:weak_form_discrete}
\begin{align}
...
b_{w}(\delta w_h,\boldsymbol q_h) &= f_{w}(\delta w_h), & \forall \delta w_h \in W_h \label{eq:force_locking_weak} \\
b(\delta \boldsymbol q_h,\boldsymbol u_h) &= f_{q}(\delta \boldsymbol q_h), & \forall \delta \boldsymbol q_h \in Q_h \label{eq:strain_locking_weak}
\end{align}
\end{subequations}
where $\boldsymbol u_h = \{w_h, \boldsymbol \varphi_h\}^T$. The related approximation spaces are defined as:
\begin{align}
    W_h &:= \mathrm{span} \{N_I^w\}_{I=1}^{n_w} \\
    \Phi_h &:= \mathrm{span} \{N_K^\varphi\}_{K=1}^{n_\varphi} \\
    Q_h &:= \mathrm{span} \{N_R^q\}_{R=1}^{n_q}
\end{align}
FIXME: approximation types
\begin{equation}
\begin{bmatrix}
\boldsymbol K^{\varphi\varphi} & \boldsymbol 0 & \boldsymbol K^{q\varphi T} \\
\boldsymbol 0 & \boldsymbol 0 & \boldsymbol K^{qwT} \\
\boldsymbol K^{q\varphi} & \boldsymbol K^{qw} & \boldsymbol K^{qq}
\end{bmatrix}
\begin{Bmatrix}
\boldsymbol d^\varphi \\ \boldsymbol d^w \\ \boldsymbol d^q
\end{Bmatrix} =
\begin{Bmatrix}
\boldsymbol f^\varphi \\ \boldsymbol f^w \\ \boldsymbol f^q
\end{Bmatrix}
\end{equation}
...
\underbrace{
\begin{bmatrix}
\boldsymbol K^{\varphi \varphi} & \boldsymbol 0 \\
\boldsymbol 0 & \boldsymbol 0
\end{bmatrix}}_{\boldsymbol K^{\mathrm{bending}}}
+
\underbrace{
\begin{bmatrix}
\tilde{\boldsymbol K}^{\varphi \varphi} & \tilde{\boldsymbol K}^{\varphi w} \\
\tilde{\boldsymbol K}^{w \varphi} & \tilde{\boldsymbol K}^{ww}
\end{bmatrix}}_{\tilde{\boldsymbol K}^{\mathrm{shearing}}}
\right)
\begin{Bmatrix}
\boldsymbol d^\varphi \\ \boldsymbol d^w
\end{Bmatrix} =
\begin{Bmatrix}
\boldsymbol f^\varphi - \tilde{\boldsymbol f}^\varphi \\
\boldsymbol f^w - \tilde{\boldsymbol f}^w 
\end{Bmatrix}
\end{equation}
\begin{align}
\tilde{\boldsymbol K}^{\varphi \varphi}&=-\boldsymbol K^{q\varphi T}(\boldsymbol K^{qq})^{-1}\boldsymbol K^{q\varphi} \\
\tilde{\boldsymbol K}^{\varphi w}&=-\boldsymbol K^{q\varphi T}(\boldsymbol K^{qq})^{-1}\boldsymbol K^{qw} \\
\tilde{\boldsymbol K}^{ww}&=-\boldsymbol K^{qw T}(\boldsymbol K^{qq})^{-1}\boldsymbol K^{qw}
\end{align}
\begin{rmk}
    As shown in Eq. \eqref{eq:weak_form_discrete}, there are two constraints in thick plate mixed formulation. One, as listed in Eq. \eqref{eq:force_locking_weak}, is a constraint mainly regarding the shear force resultant on natural boundary $\Gamma_q$ and its divergence in entire domain.
    The other is deflection essential boundary condition $\Gamma_w$ and the shear strain enforcement in whole domain, as listed in Eq. \eqref{eq:strain_locking_weak}.
    However, for the most of pioneer works, only consider the shear strain constraint, and that is not sufficient for a satisfaction result.
\end{rmk}
